...
def index(request):
    ...
    for cat in cats:
        ...
        for p in [i for i in Product.objects.filter(category=cat)]:
            prod.append(
                [p, [item for item in ProductSize.objects.filter(product=p)]])
        n = len(prod)
        nSlides = 5
...
def view_all(request, catg):
    if catg == 'dow':
        ...
        for p in trend.objects.order_by('number'):
            prod.append(
                [p.product, [item for item in ProductSize.objects.filter(product=p.product)]])
        params = {
            'product': prod,
            'catg': 'Treanding',
            'cart_element_no': len([p for p in Cart.objects.all() if p.user == request.user]),
...
def search(request):
    ...
    for prod in [i for i in Product.objects.all()]:
        if query.lower() in prod.product_name.lower() or query.lower() in prod.desc.lower() or query.lower() in prod.subcategory.lower():
            prods.append(
                [prod, [item for item in ProductSize.objects.filter(product=prod)]])
    params = {
        'product': prods,
        'cart_element_no': len([p for p in Cart.objects.all() if p.user == request.user]),
        'category': category.objects.all(),
                ...
                for item in prod_list:
                    ...
                    if pppp[0] in card_prods_id:
                        cart_prods[card_prods_id.index(
                            pppp[0])].number = int(pppp[1])
                        cart_prods[card_prods_id.index(pppp[0])].save()
                    else:
                        Cart(user=request.user,
                             product_id=pppp[0], number=int(pppp[1])).save()
        else:
...
def cart(request):
    if request.user.is_authenticated:
        ...
        for p in cart_prods:
            ...
            subtotal += tempTotal
            tax += tempTotal * \
                int(Product.objects.filter(product_id=p.product_id).first().gst)/100
...
def add_to_cart(request):
    ...
    if request.method == 'GET':
        ...
        Cart(user=request.user, product_id=int(
            prod_id[0]), product_size=prod_id[1], number=1).save()
...
def order_now(request):
    ...
    if request.method == 'POST':
        ...
        address_form = UserAddressForm(
            request.POST, instance=request.user.userdetail)
...
# 			print(order)
            if pay_mode == 'on':
                ...
                Orders(order_id=order_id, user=request.user, saler=Product.objects.filter(
                    product_id=int(new_prod)).first().shop, products=product1, size=prod_size).save()
                if int(new_prod) in trends:
                    t = trend.objects.filter(product=Product.objects.filter(
                        product_id=int(new_prod)).first())[0]
                    t.number += 1
                    t.save()
                else:
                    trend(product=Product.objects.filter(
                        product_id=int(new_prod)).first(), number=1).save()
                ...
                if int(new_prod) in trends:
                    ...
                    trend(product=Product.objects.filter(
                        product_id=int(new_prod)).first(), number=1).save()
                delev = 0.0
                subtotal = Product.objects.filter(
                    product_id=int(new_prod)).first().price
                tax = subtotal * \
                    int(Product.objects.filter(
                        product_id=int(new_prod)).first().gst)/100
    ...
    tax = subtotal * \
        int(Product.objects.filter(product_id=int(new_prod)).first().gst)/100
...
# 			print(order)
            if pay_mode == 'on':
                for item in cart_prods:
                    if Orders.objects.all().last():
                        order_id = 'ordr' + \
                            str((Orders.objects.all().last().pk)+1)
                    else:
                        order_id = 'ordr001'
                    product1 = item.product_id+'|'+str(item.number)+','
                    Orders(order_id=order_id, user=request.user, saler=Product.objects.filter(product_id=int(
                        item.product_id)).first().shop, products=product1, size=item.product_size).save()
                    item.delete()
                    if int(item.product_id) in trends:
                        t = trend.objects.filter(product=Product.objects.filter(
                            product_id=int(item.product_id)).first())[0]
                        t.number += 1
                        t.save()
                    else:
                        trend(product=Product.objects.filter(
                            product_id=int(item.product_id)).first(), number=1).save()
                return redirect('/myorders')
            else:
                temp = 1
    else:
    ...
    if temp == 1:
        ...
        for item in cart_prods:
            ...
            product1 = item.product_id+'|'+str(item.number)+','
            Orders(order_id=order_id, user=request.user, saler=Product.objects.filter(
                product_id=int(item.product_id)).first().shop, products=product1, size=item.product_size)
            ...
            if int(item.product_id) in trends:
                ...
                trend(product=Product.objects.filter(
                    product_id=int(item.product_id)).first(), number=1)
        param_dict = {
...
def contact(request):
    if request.method == 'POST':
        ...
        con = Contact(name=cont_name, email=cont_email,
                      subject=cont_subject, message=cont_mess)
        con.save()
